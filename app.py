...
# සර්වර් එකට ඇත්තටම අහුවුණු ෆයිල් ටික වෙබ් එකේම පෙන්නමු (Debugging)
st.info(f"📁 GitHub එකෙන් සර්වර් එකට ලැබුණු ෆයිල්: {os.listdir('.')}")
...
def load_model():
    try:
        ...
        # Error එකක් ආවොත් හිරවෙන්නේ නැතුව ඒක වෙබ් එකට යවනවා
        return str(e)

model = load_model()

# Model එක ලෝඩ් වුණාද නැද්ද කියලා පරීක්ෂා කිරීම
if isinstance(model, str):
    st.error(f"🚨 Model එක Open වෙද්දී ආපු Error එක: {model}")
else:
    st.success("✅ Model එක සාර්ථකව Open වුණා!")
    st.write("ඔබේ වගාවේ රෝගී වූ කොළයක පින්තූරයක් මෙතනට Upload කරන්න.")

    # පින්තූරය Upload කිරීමට අවකාශය
